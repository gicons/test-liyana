     ...
     <%       
          try{
        	  Connection conn = null;
		       ...
		       rs=st.executeQuery("select * from orders where custId = '"+id+"'");
		       while (rs.next()){          
     %>
     <tr>
...
<th>Sub Total </th>
     ...
     try{
 	    ...
 	    st = conn.createStatement();
	        rs=st.executeQuery("select *from product inner join cart on product.productId=cart.product_id inner join customer on cart.cust_id=customer.custId inner join orders on customer.custId=orders.custId where customer.custId ='"+id+"'");
	       while (rs.next()){          
     %>
<tr>
...
<table>
 <% 
          ...
          try{
      	    ...
      	    st = conn.createStatement();
		        rs=st.executeQuery("select sum(cartTotalPrice),orders.orderId from product inner join cart on product.productId=cart.product_id inner join customer on cart.cust_id=customer.custId inner join orders on customer.custId=orders.custId where customer.custId ='"+id+"'");
		       while (rs.next()){
